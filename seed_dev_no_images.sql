-- Development seed data for Hogtech e-commerce (NO IMAGES VERSION)
-- Safe to run multiple times; uses conditional inserts to avoid duplicates.
-- This version uses NULL for images until you upload real ones.
...
SELECT 'Laptops', 'laptops', 'Performance notebooks for professionals and students.', NULL, NULL, TRUE, 'left', 1, 0
WHERE NOT EXISTS (SELECT 1 FROM public.categories WHERE slug = 'laptops');

INSERT INTO public.categories (name, slug, description, image_url, parent_id, show_in_mega_menu, mega_menu_column, "order", product_count)
SELECT 'Accessories', 'accessories', 'Keyboards, mice, hubs and more.', NULL, NULL, TRUE, 'right', 2, 0
...
SELECT 'Back to Campus Deals', 'Save up to 15% on student essentials all month long.', NULL, '/shop/deals', 'banner', 'homepage', now(), now() + interval '30 days', TRUE
WHERE NOT EXISTS (SELECT 1 FROM public.promotions WHERE title = 'Back to Campus Deals');

-- Products (with NULL images - you'll upload real ones later)
